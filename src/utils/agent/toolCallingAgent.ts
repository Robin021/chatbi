...
import { FetchedData, emptyFetchedData } from "@/utils/dataSource";

export const createToolCallingAgent = (agent: Omit<Agent, 'run'>): Agent => {

    const run = async (history: ChatMessage[], actions: ChatActions, inputData: Partial<Record<ChatPipelineInput, any>>, finishedCallback: () => void) => {
